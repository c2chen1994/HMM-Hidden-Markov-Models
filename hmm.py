...
def forward(pi, A, B, O):
  ...
  ###################################################
  # Q3.1 Edit here
  #base case: alfha(s,1) = pi(s) * b(s,x1)
  alpha[:,0] = pi * B[:,O[0]]
  for i in range(1,N):
    alpha[:,i] = B[:,O[i]] * np.dot(alpha[:,i-1], A)
  ###################################################
...
def viterbi(pi, A, B, O):
  ...
  c[:,0] = pi * B[:,O[0]]
  for i in range(1,N):
    c[:,i] = B[:,O[i]] * np.max(np.tile(c[:,i-1].reshape(S,1),(1,S)) * A, axis=0)
    d[:,i] = np.argsort(-(np.tile(c[:,i-1].reshape(S,1),(1,S)) * A), axis=0)[0]
  #backtracking:
  z = np.argsort(-c[:,N-1])[0]
  path.insert(0,z)
